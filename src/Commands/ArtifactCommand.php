...
class ArtifactCommand extends Command {
  ...
  /**
   * Flag to fail artifact build when branch is missing.
   *
   * By default (false), artifact build is skipped with exit code 0.
   * When true, artifact build fails with exit code 1.
   */
  protected bool $failOnMissingBranch = FALSE;

  /**
   * Flag indicating artifact build was skipped due to missing branch.
   */
  protected bool $buildSkipped = FALSE;
  ...
  protected function configure(): void {
    ...
    $this
      ...
      ->addOption('fail-on-missing-branch', NULL, InputOption::VALUE_NONE,     'Fail artifact build if source branch cannot be determined. By default, artifact build is skipped gracefully.');
    // @formatter:on
  ...
  protected function execute(InputInterface $input, OutputInterface $output): int {
    ...
    try {
      ...
      // Check if artifact build was skipped due to missing branch.
      if ($this->buildSkipped) {
    ...
    $this->output->writeln('<info>Artifact build finished successfully.</info>');
  ...
  protected function resolveOptions(string $url, array $options): void {
    ...
    try {
      ...
      if ($this->failOnMissingBranch) {
        // Strict mode: fail artifact build.
        throw new \RuntimeException('Unable to determine source branch. Artifact build failed. ' . $exception->getMessage(), $exception->getCode(), $exception);
      }

      $commit_hash = $exception->getCommitHash();
      $this->logger->notice('Source branch not found. Artifact build will be skipped.');
      $this->logger->notice('Commit: ' . $commit_hash);

      $this->output->writeln('<comment>Source branch not found. Artifact build skipped.</comment>');
      $this->output->writeln('<comment>Commit: ' . $commit_hash . '</comment>');
      $this->output->writeln('<info>Use --fail-on-missing-branch to fail artifact build instead.</info>');

      // Set flag to skip artifact build and return early from execute().
